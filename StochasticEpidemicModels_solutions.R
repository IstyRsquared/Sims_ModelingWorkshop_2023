...
## for transmission rate parameter beta=2.8, or and recovery rate sigma=1/3 (infectious period of 3 weeks)
sigma <- 1/3
beta <- 2.8
S0 <- 999 
I0 <- 1
...
## b) Demography
## Add birth and death into your model, run over 100 days
...
for(run in 1:100){
  ...
  outbreak.duration <-c(outbreak.duration, t)
}

par(mfrow=c(1,1))
hist(outbreak.duration, main = NULL) 
summary(outbreak.duration) # ~20 days
...
while (( N > 0) && ( S > 0) && (I > 0)){
  r <- c( beta * S * I/N, sigma * I, muS * S, muI * I, muR * R, alpha * R)
  rtotal <- sum(r)
  cr <- cumsum(r)
  delta <- (-1/rtotal)*log(runif(1))
  t <- t + delta # Time to the next event
...
## c) Vaccination
# EX3. Modify the stochastic SIR model (no demography) to have vaccination with rate gamma=20 (individuals per time step)
gamma <- 20
S0 <- 999 
I0 <- 1
R0<- 0
...
while (( I > 0) && ( S > 0)){
  ...
  if (P < cr[1]) {
    ...
    S <- max(0, S - 1) # Vaccination
    R <- max(0, R + 1)
    V <- V+1 # Keep track of of individuals vaccinated
